...
changeSides state =
  let (attacker, defender) = fightCombatants state
   in state {fightCombatants = (defender, attacker)}

-- | Run the PC's attack with a specific spell.
runPCAttack :: Spell -> FightState -> FightStatus
runPCAttack spell state =
  let spentState = spendSpell spell state
   in case spellDuration spell of
        ...
        0 ->
          let effectedCombatants =
           ...
           in checkForDead $ changeSides effectedState
        _ ->
          let effects = fightEffects spentState
           in Right $ changeSides $ spentState {fightEffects = spell : effects}
...
runFight state0 =
  let soln =
        find (isLeft . maximum . S.toList) $
          iterate processRound $
            S.singleton (Right state0)
   in case soln of
...
tryRound status spell = do
  ...
  let status' = do
        ...
        let (attacker, _) = fightCombatants state
        when
          (statsType attacker /= PC)
          (error "flipped try round")
        effectedState <- runEffects state
        unless
          (spell `elem` availableSpells effectedState)
