...
process :: [(Point, [Point])] -> RWS [Point] () (HashMap Point [Point]) ()
process newPoints = do
  ...
  let pointsToRemove = (map head . filter ((/= 1) . length) . group . sort) allNewPoints
  allExistingPoints <- gets (concat . HM.elems)
  let newPoints' = map (second (filter (`elem` allExistingPoints) . filter (`elem` pointsToRemove))) newPoints
  modify (HM.unionWith (<>) (HM.fromList newPoints'))
  rs <- ask
  if all (null . fromJust . (`lookup` newPoints')) rs then return () else process newPoints'

part1 :: [Point] -> [Point] -> HashMap Point [Point]
part1 rs ps =
  let input = map (\p -> (p, getNeighbors4P p)) ps
      ss = HM.fromList $ map (\p -> (p, [p])) ps
   in fst $ execRWS (process input) rs ss

main' :: IO ()
main' = do
  contents <- map parse . lines <$> readFile "../inputs/2018/Day6/sample.txt"
  let es = findEncapsulations contents
  let r1 = part1 es contents
