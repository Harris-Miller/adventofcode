...
const data = (await Bun.file('../inputs/2024/Day6/input.txt').text()).trim().split('\n');
...
function tryNewMap(new_area, new_obstacles, x, y, direction) {
  while (true) {
    const new_x = x + move[direction][0];
    const new_y = y + move[direction][1];
  ...
  while (true) {
    ...
    if (area[new_x][new_y] === '#') {
      ...
      if (area[new_x][new_y] !== 'X') {
        const new_area = area.map(line => [...line]);
        new_area[new_x][new_y] = '#';

        const new_obstacles = new Map();
